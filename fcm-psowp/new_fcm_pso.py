...
Optimizing centroid using FCM style. 
In hybrid mode will use FCM to seed first particle's centroid'''
...
class PSO:
    ...
    def show_cluter(self):
        ...
        '''mark = ['o', 'o', 'o', 'o', 'o', '+', 's', 'd', '<', 'p']
        color = ['r', 'b', 'g', 'm', 'c', 'y']
        n, m = self.data.shape
        final_u = np.max(self.gbest_position, axis=1)

        fig = plt.figure()
        ax = Axes3D(fig)

        for cent0, cent1 in self.final_centroid:
            ax.scatter(cent0, cent1, c='k', marker='D')

        for i in range(n):
            markIndex = int(self.cluster[i])
            ax.scatter(self.data[i, 0], self.data[i, 1], zs=0, zdir='z', c=color[markIndex],
                       marker=mark[markIndex], alpha=0.6)

        ax.plot(self.data[:, 0], final_u, zs=0, zdir='y', c='brown', label='Membership')
